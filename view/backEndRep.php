    ...
    <div class="wrapper">
      ...
      <div class="main-container">
        ...
        <div class="content-wrapper" id="sheesh" >
          <table class="table mt-0 table-striped" style="width:100%">
          <thead>
            <tr>
                <th scope="col-2">Reporter</th>
                <th scope="col-2">Reported</th>
          ...
          <tbody>
              ...
              foreach($reports AS $report)
            {
              ...
              else 
              {
                ...
                echo '<tr scope="row">
                <th scope="col" class="col-2">'.$report["reporter"]["username"].'</th>
                <th scope="col" class="col-2">'.$report["reported"]["username"].'</th>
                <th scope="col" class="col-5" style="word-break:break-all">'.$report["message"].substr(0,50).'</th>';
                if($report["action"]=="removed")
                echo' <th scope="col" class="col-3"><div>removed</div> <button id="'.$report["reported"]["id"].'" name="'.$report["id"].'" onclick=takeAction(this)>Ban '.$report["reported"]["username"].'</button></th>';
                else {
                  if($report["action"]=="banned")
                  echo' <th scope="col" class="col-3"><div>banned</div><button id="'.$id.'" name="'.$report["id"].'" onclick=takeAction(this)>'.$action.'</button></th>';
                  else
                  {
                    if($report["action"]=="removed-banned")
                    echo' <th scope="col" class="col-3">removed banned</th>';
                    else  echo' <th scope="col" class="col-3"><div></div><button id="'.$id.'" name="'.$report["id"].'" onclick=takeAction(this)>'.$action.'</button><button id="'.$report["reported"]["id"].'" name="'.$report["id"].'" onclick=takeAction(this)>Ban '.$report["reported"]["username"].'</button></th>';
                  }
  ...
  <script>
    function takeAction(action)
    {
        ...
        $.when($.ajax({
          ...
          {
            action.previousSibling.innerText="removed";
            action.remove();

          }
          if(data.trim()=="banned")
          {
            action.previousSibling.innerText="banned";
            action.remove();
          }
          if(data.trim()=="removed-banned")
          {
            action.parentElement.innerHTML="removed banned";
            action.remove();
          }
